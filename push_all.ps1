...
foreach ($city in $cities) {
    ...
    if (-not (Test-Path $path)) {
        ...
        $cityName = $city -replace '-local$', ''
        $path = "C:\Users\DIEGO\Desktop\moweb\$cityName\index.html"
    }
    ...
    try {
        ...
        $sha = $current.sha
        $body = @{message = "fix: reemplazar ultimos emojis corruptos con Font Awesome"; content = $content; sha = $sha} | ConvertTo-Json
